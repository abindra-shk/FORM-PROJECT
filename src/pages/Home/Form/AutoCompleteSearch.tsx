...
const AutoCompleteSearch: React.FC<AutoCompleteSearchProps> = ({
  ...
  const debouncedInputChange = useCallback(debounce((value: string) => {
    ...
    if (filteredOptions.length === 0 && value.trim() !== '') {
      setOptions([{
        name: `${value}`, _id: 'add', ratePerHour: 500,
        email: "",
        address: "",
        hours: 0,
        total: 0,
  ...
  const handleOptionSelect = (event: React.SyntheticEvent, value: FormItem | null) => {
    event.preventDefault()
    ...
    <Autocomplete
      ...
      renderOption={(props, option) => (
        <li {...props} key={option._id}>
          {option.name} {option._id === 'add' ? '(Add New Record)' : `(${option.ratePerHour} $/hr)`}
        </li>
      )}
